...
This script takes as input the path to the original BIDS dataset and outputs a datalist json file
    ...
    uses pre-defined splits by default (they can be found under the folder <path-to-repo/datasplits>.)

Modified from nnUnet/02_create_msd_data.py:
    - git-annex downloads are batched into a single `git annex get` call instead of one per file
    - get_image_stats removed (reads full NIfTI data, not needed for datalist creation)
...
def create_df(args, dataset_path):
    ...
    # Build the list of files not yet downloaded (broken symlinks have os.path.exists=False)
    files_to_get = []
    for fname_label in df['filename']:
        ...
        if not os.path.exists(fname_label):
            files_to_get.append(fname_label)
        if not os.path.exists(fname_image):
            files_to_get.append(fname_image)

    if files_to_get:
        file_list = ' '.join(f'"{f}"' for f in files_to_get)
        subprocess.run(f'cd {dataset_path}; git annex get {file_list}', shell=True, check=True)
        logger.info(f"Downloaded {len(files_to_get)} files from git-annex")
    else:
        logger.info("All files already present on disk, skipping git-annex get")    
