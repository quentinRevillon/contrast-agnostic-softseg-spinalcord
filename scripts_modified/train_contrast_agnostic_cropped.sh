#!/bin/bash
# Training script for contrast-agnostic v3.0 with asymmetric SC bbox crop (Dataset997).
#
# Pipeline:
...
#   5. Train
#
# Dataset997: SC bbox cropped with asymmetric margins:
#   LR ±20mm | AP ±30mm | Sup +40mm | Inf → bottom of image
# Dataset998: SC bbox cropped with uniform 10mm padding (previous experiment)
# Dataset999: full images, no SC crop (original pipeline)
#
# Usage (via set_slot):
#   set_slot 0 /bin/bash /home/quentinr/contrast-agnostic-softseg-spinalcord/scripts_modified/train_contrast_agnostic_cropped.sh
...
DATASET_NAME="ContrastAgnosticAsymCrop"
DATASET_NUMBER=997

# Asymmetric SC bbox margins (all in mm)
PAD_LR=20.0    # left and right
PAD_AP=30.0    # anterior and posterior
PAD_SUP=40.0   # superior only; inferior extends to the bottom of the image
...
# ====================================
# PHASE 3 — CONVERT TO NNUNET FORMAT + ASYMMETRIC SC BBOX CROP
# ====================================
# Produces Dataset997_ContrastAgnosticAsymCrop in nnUNet_raw.
# Takes ~8-10h with 8 workers.

echo "-----------------------------------"
echo "Converting datalists to nnUNet format with asymmetric SC bbox crop..."
echo "  LR: ±${PAD_LR}mm  |  AP: ±${PAD_AP}mm  |  Sup: +${PAD_SUP}mm  |  Inf: to bottom"
echo "-----------------------------------"
